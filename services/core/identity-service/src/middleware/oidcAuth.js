...
// OIDC bearer-token auth for identity-service, following the same pattern
// used across the other core services (see e.g.
// services/core/samba-ad-dc/src/middleware/oidcAuth.js and
// services/core/least-privilege/src/middleware/oidcAuth.js): verify a
// Bearer JWT against the platform JWKS, then a separate role-gate
// (requireAdmin) for endpoints that mutate the directory — user, group, and
// OU create/update/delete. A verified JWT only proves *who* is asking;
// those endpoints additionally require an admin role, since identity-service
// is the platform's identity store and unauthenticated (or merely
// authenticated-but-unprivileged) directory mutation is the exact gap this
// middleware closes.
...
// fallback) that grants admin access to this service's directory-mutation
// endpoints (users/groups/OUs create, update, delete, group-membership).
const ADMIN_ROLES = ['admin'];
// Scope claim (space-delimited req.user.scope string, or an array under
// req.user.scopes) that grants the same access.
const ADMIN_SCOPES = ['identity.admin'];

/**
 * True if the verified-JWT payload carries a role or scope authorized to
 * mutate the directory (users, groups, OUs). Defensive by design: any shape
 * mismatch (missing claims, wrong types) resolves to false rather than
 * throwing — the caller (requireAdmin) must fail closed.
 */
 ...
 * create, update, or delete users, groups, or OUs (and group-membership
 * mutations). Requires req.user (set by oidcAuth from a verified JWT) to
 * carry an admin role or an identity.admin scope. Fails closed on a
